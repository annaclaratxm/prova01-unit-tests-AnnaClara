...
describe("Teste da classe Utilitarios", () => {
  ...
  test("Deve inverter uma string corretamente", async () => {
    const stringInvertida = utilitarios.inverterString('João');
    expect(stringInvertida).toBe('oãoJ');
  });

  test("Deve contar a quantidade de caracteres corretamente", async () => {
    const stringContada = utilitarios.contarCaracteres('João');
    expect(stringContada).toBe(4);
  });

  test("Deve converter todas as letras para maiúsculas", async () => {
    const stringConvertidaMaiuscula = utilitarios.paraMaiusculas('João');
    expect(stringConvertidaMaiuscula).toBe('JOÃO');
  });

  test("Deve converter todas as letras para minúscula", async () => {
    const stringConvertidaMinuscula = utilitarios.paraMinusculas('JOÃO');
    expect(stringConvertidaMinuscula).toBe('joão');
  });

  test("Deve converter a primeira letra para maiúscula", async () => {
    const stringVerificadaMaiuscula = utilitarios.primeiraLetraMaiuscula('joão');
    expect(stringVerificadaMaiuscula).toBe('João');
  });
  ...
  test("Deve verificar se número negativo é par", async () => {
    const resultado = utilitarios.ehPar(-4);
    expect(resultado).toBe(true);
  });

  test("Deve verificar se número negativo é ímpar", async () => {
    const resultado = utilitarios.ehPar(-5);
    expect(resultado).toBe(false);
  });
  ...
  test("Deve verificar se o valor é NaN", async () => {
    const resultado = utilitarios.ehNumero(NaN);
    expect(resultado).toBe(false);
  });

  test("Deve remover espaços da string", async () => {
    const resultado = utilitarios.removerEspacos("  João  ");
    expect(resultado).toBe("João");
  });

  test("Deve remover espaços de uma string vazia", async () => {
    const resultado = utilitarios.removerEspacos("");
    expect(resultado).toBe("");
  });

  test("Deve remover espaços de uma string sem espaços", async () => {
    const resultado = utilitarios.removerEspacos("Teste");
    expect(resultado).toBe("Teste");
  });

  test("Deve repetir texto corretamente", async () => {
    const resultado = utilitarios.repetirTexto("Olá", 3);
    expect(resultado).toBe("OláOláOlá");
  });
  ...
  test("Deve contar palavras corretamente", async () => {
    const resultado = utilitarios.contarPalavras("Olá mundo teste");
    expect(resultado).toBe(3);
  });

  test("Deve contar palavras corretamente com múltiplos espaços", async () => {
    const resultado = utilitarios.contarPalavras("Olá    mundo   teste");
    expect(resultado).toBe(3);
  });
  ...
  test("Deve verificar se string é palíndromo", async () => {
    const resultado = utilitarios.ehPalindromo("radar");
    expect(resultado).toBe(true);
  });

  test("Deve verificar se string com caracteres especiais é palíndromo", async () => {
    const resultado = utilitarios.ehPalindromo("A man, a plan, a canal: Panama");
    expect(resultado).toBe(true);
  });
  ...
  test("Deve mesclar objetos e sobrescrever chaves duplicadas", async () => {
    const obj1 = { a: 1, b: 2 };
    const obj2 = { b: 3, c: 4 };
    const resultado = utilitarios.mesclarObjetos(obj1, obj2);
  ...
  test("Deve gerar número aleatório dentro do intervalo correto", async () => {
    const numeroAleatorio = utilitarios.gerarNumeroAleatorio(10);
    expect(numeroAleatorio).toBeGreaterThanOrEqual(0);
    expect(numeroAleatorio).toBeLessThan(10);
  });
});
